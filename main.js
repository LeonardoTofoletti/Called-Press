...
// ---- IA Sugestão em tempo real ----

let timeoutSugestao;
let ultimaSugestao = '';

const camposIASugestao = [
  'problemCause',
  'resolution',
  'duvidaCliente',
  'duvidaExplicacao',
...
camposIASugestao.forEach(id => {

  const campo = document.getElementById(id);

  if (!campo) return;

  // cria elemento visual da sugestão
  const sugestao = document.createElement('div');

  sugestao.style.fontSize = '13px';
  sugestao.style.opacity = '0.7';
  sugestao.style.marginTop = '4px';
  sugestao.style.fontStyle = 'italic';
  ...
  campo.parentNode.appendChild(sugestao);
  ...
  campo.addEventListener('input', () => {
    ...
    // evita gastar IA com texto pequeno
    ...
    timeoutSugestao = setTimeout(async () => {
      ...
      try {
        ...
        // evita repetir igual
        if (data.resultado.trim() === texto.trim()) return;

        ultimaSugestao = data.resultado;

        sugestao.innerHTML =
          `💡 Sugestão IA: ${data.resultado.substring(0, 180)}... <b>(clique para aplicar)</b>`;
